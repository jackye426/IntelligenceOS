...
2. **Per clinic:** fuzzy-match Doctify `clinic_name` + `location` (postcode) against that local CSV — this is an in-memory lookup, not an API crawl of every CQC location.
3. **On match:** HTTP-scrape **that one** CQC location Overview page for Registered Manager / Nominated Individual.

So we pull the **directory once**, then **look up** each Doctify clinic. We do **not** scrape every CQC location in England.

`gtm-pipeline cqc match` uses the same directory file with numeric `match_confidence`. `gtm-pipeline cqc location` scrapes one Overview URL after you already know the location id.
