...
RELATIONSHIP_GMAIL_SEND_AS_ALIASES=admin@docmap.co.uk
...
`RELATIONSHIP_GMAIL_SEND_AS_ALIASES`. In Google Workspace, each alias must be a verified
**Send mail as** address on the `admin@docmap.co.uk` account before Railway env vars will work.

Aliases also feed reply detection (`sync_replies`): outbound mail from those addresses is
treated as "from us".

### Production Railway (Email MCP service)

Set explicitly (triggers redeploy):
...
### Gmail OAuth scopes and token remint

The refresh-token helper requests only `gmail.readonly` and `gmail.compose` (no `gmail.modify`).
`gmail.compose` still allows sending mail via the API; draft-only behaviour is enforced by
`RELATIONSHIP_DESK_MODE`, not by OAuth scope alone.

After changing scopes, remint as the mailbox owner:

```bash
# Place client_secret.json at IntelligenceOS repo root, then:
python relationship-desk-mcp/generate_gmail_refresh_token.py
# Update RELATIONSHIP_GMAIL_REFRESH_TOKEN on the Email MCP Railway service.
```
