...
| A6 | **Clinic sales CSV** | Live (2026-07-04) | internal | `clinic_accounts` (seed/enrich) + draft `clinic_contacts` | `clinic_account` | `Clinic sales agent/output/clinic_sales_results.csv` | `python -m ingestion_pipeline sync clinic-csv` (P4 lane; insert-only, idempotent) | Re-run when CSV refreshed; manual edits never overwritten |
