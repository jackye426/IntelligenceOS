...
Internal intelligence platform for **DocMap**: clinic outreach and account intelligence, a hosted MCP for Claude, TikTok marketing ETL, and background data workers â€” one monorepo, shared Supabase store.
...
- **Clinic OS (Next.js)** â€” outreach / accounts UI over `clinic_accounts` and related tables
- **MCP server** â€” TikTok tools, decision log, document search, Gmail draft outreach (`confirmed` required)
- **Marketing pipeline** â€” TikTok catalog â†’ transcripts / OCR / comments â†’ component extract â†’ Supabase
- **Data worker** â€” Railway cron for TikTok refresh and metric layers
- **Ingestion pipeline** â€” clinic CSV seed into Supabase with embeddings
...
| `ingestion-pipeline/` | Clinic CSV â†’ Supabase |
...
Copy `.env.example` â†’ `.env.local`. Common model knobs:

- `MODEL_OCR` â€” vision OCR (default Gemini Flash)
- `MODEL_COMPONENTS` â€” component cards (default DeepSeek flash)
...
Ops and TikTok M3 paths are largely live (MCP healthy, clinic import seeded). Instagram module and full dashboard Vercel promotion are still in flight â€” see `STATUS.md` for the dated checklist.
