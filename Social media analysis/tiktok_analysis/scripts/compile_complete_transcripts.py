...
def main() -> None:
    ...
    ap.add_argument(
        "--no-comments",
        action="store_true",
        help="Skip comment section entirely",
    )
    ap.add_argument(
        "--live-comments",
        action="store_true",
        help="Fetch comments live from TikTok API (default: read comments_raw/ cache)",
    )
    ap.add_argument(
        "--top-comments",
        type=int,
    ...
    blocks = [
        ...
        f"# Comments source: {'live API' if args.live_comments else 'comments_raw cache'}",
        "",
    ]
